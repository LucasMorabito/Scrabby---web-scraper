...
def fetch_api_results(query: str, limit: int = 50) -> list[dict]:
    ...
    response = requests.get(API_URL, params=params, timeout=15, headers=_headers("application/json"))
...
def scrape_search(query: str = "placas de video", limit: int = 50) -> list[dict]:
    try:
        products = parse_products_api(fetch_api_results(query, limit=limit))
